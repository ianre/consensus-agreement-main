...
def main(): 
    ...
    #print(task)
    task = "Multiple" #then suturing to b done w/ jigs
    I = Iterator(task)
    #I.fixStates()
    #I.poll()
    #I.verifyOutput()
...
class Iterator:
            ...
            for file in files:
                ...
                with open(ian_file) as ian_data:
                    for line in ian_data:
                        ian_lines.append(line.strip())
                if("Post" in file):
                    kay_lines = self.takeAwayAngles(kay_lines)
                #print("Here")
                #ian_lines = self.takeAwayAngles(ian_lines)
                #v_lines = []
                #with open(v_file) as v_data:
                ...
                for line in kay_lines:
                    ...
                    outPollRow = self.majority_voting(line, ian_line,i)
    ...
    def pollLine_cohen_kappa(self, k_line, i_line,lineNumber, DEBUG=True,DEBUG_INFO="none", ):
        ...
        for i in range (1,len(k_s)):
            ...
            candidate = self.majority(k_s[i], i_s[i]);
            o_s[i] = candidate

        if(DEBUG):

            line = k_s[0] + " " + "".join(k_s[1:])            
            line = line + " "+self.lineToStr(i_line)+":" +"".join(o_s[1:])      #+ str(res)
        else:
            line = " ".join(k_s)
    ...
    def checkUniqueness(self, k, i, line):
        global unique_labels
    ...
    def majority_voting(self, k_line, i_line,lineNumber):     
        k_s = k_line.split(" ")
        i_s = i_line.split(" ")
        #v_s = v_line.split(" ")
